# axiam-sdk (Java)

Official Java client SDK for [AXIAM](https://github.com/axiam/axiam) — Access eXtended Identity and Authorization Management.

## Package identity

- **Maven coordinates:** `io.axiam:axiam-sdk`
- **GroupId:** `io.axiam`
- **ArtifactId:** `axiam-sdk`
- **Registry:** Maven Central _(reserved, not yet published)_
- **License:** Apache-2.0

## Contract conformance

This SDK conforms to CONTRACT.md §1-§10.

See [`../CONTRACT.md`](../CONTRACT.md) for the full cross-language behavioral contract.

## Status

Scaffold placeholder. Full implementation follows in Phase 20 (Java SDK).

## Usage

```xml
<dependency>
  <groupId>io.axiam</groupId>
  <artifactId>axiam-sdk</artifactId>
  <version>0.1.0</version>
</dependency>
```
