...
public enum AuditOutcome {
    /** The server's 'Success' value. */
    SUCCESS("Success"),
    /** The server's 'Failure' value. */
    FAILURE("Failure"),
    /** The server's 'Denied' value. */
    DENIED("Denied");

    /** The spelling this value has on the wire. */
    private final String wire;

    AuditOutcome(String wire) {
        this.wire = wire;
    }
     ...
     * @return the matching AuditOutcome
     * @throws IllegalArgumentException if no constant carries that spelling
     */
    @JsonCreator
    public static AuditOutcome fromWire(String value) {
        for (AuditOutcome candidate : values()) {
            if (candidate.wire.equals(value)) {
                return candidate;
            }
        }
        throw new IllegalArgumentException("unknown AuditOutcome value: " + value);
    }
}
