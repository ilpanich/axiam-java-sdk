...
public enum MfaMethodType {
    /** The server's 'Totp' value. */
    TOTP("Totp"),
    /** The server's 'Passkey' value. */
    PASSKEY("Passkey"),
    /** The server's 'SecurityKey' value. */
     ...
     * @return the matching MfaMethodType
     * @throws IllegalArgumentException if no constant carries that spelling
     */
    @JsonCreator
    public static MfaMethodType fromWire(String value) {
        for (MfaMethodType candidate : values()) {
            if (candidate.wire.equals(value)) {
                return candidate;
            }
        }
        throw new IllegalArgumentException("unknown MfaMethodType value: " + value);
    }
}
