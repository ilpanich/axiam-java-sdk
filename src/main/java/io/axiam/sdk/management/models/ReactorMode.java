...
public enum ReactorMode {
    /** The server's 'intercept' value. */
    INTERCEPT("intercept"),
    /** The server's 'listen' value. */
    LISTEN("listen");

    /** The spelling this value has on the wire. */
    private final String wire;

    ReactorMode(String wire) {
        this.wire = wire;
    }
     ...
     * @return the matching ReactorMode
     * @throws IllegalArgumentException if no constant carries that spelling
     */
    @JsonCreator
    public static ReactorMode fromWire(String value) {
        for (ReactorMode candidate : values()) {
            if (candidate.wire.equals(value)) {
                return candidate;
            }
        }
        throw new IllegalArgumentException("unknown ReactorMode value: " + value);
    }
}
