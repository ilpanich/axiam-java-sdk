...
- **CONTRACT.md §10.4 — an optional session-revocation feed poller (contract
  1.44).** `io.axiam.sdk.internal.RevocationFeed`, passed to a new
  three-argument `JwksVerifier(baseUrl, policy, revocationFeed)` constructor.
  ...
  Nothing changes unless you pass one: the one- and two-argument constructors
  are untouched and remain the default. It never fetches on the request path
  after the first call — `verifyAccessToken` reads a cached set. And it never
...
- Two tests pinning CONTRACT.md §16 against the server's new answer for a
  ...
  changed: §16.3 already retried `5xx` on an eligible operation and `Retry` —
  the implementation whose parameters the contract adopted — already honoured
  `Retry-After` as a floor. Both halves are asserted through the public surface
  with a wire count, because a retry policy nobody exercises that way is the
  failure §16.7 exists for.
...
- **RFC 8705 §5 `mtls_endpoint_aliases` (SDK contract 1.40, CONTRACT.md §21.3
  ...
  §12 helpers now prefer an alias over the top-level entry of the same name on
  any call made over mutual TLS — that is, from a client built with
  `clientCertificate(...)`. Six operations reach the token endpoint
  (`oidcExchange`, `oidcRefresh`, `loginClientCredentials`, `devicePoll`,
  `tokenExchange`, `umaExchangeTicket`), plus `introspect`, `revoke`,
  `deviceAuthorize` and `oidcPar`.

  A null component means "this deployment terminates mutual TLS on the issuer's
  own host", never "mTLS is unsupported": a client without it keeps using the
  conventional endpoints instead of failing. Every component of
  `MtlsEndpointAliases` is `@Nullable`, so an endpoint a partial object does
  ...
  `OidcConfiguration` keeps a constructor at its pre-1.40 arity, defaulting the
  new component to `null`, so existing `new OidcConfiguration(...)` call sites
  still compile.
...
- **Contract 1.38: the four public "Sign in with X" operations.** `ssoProviders`,
  ...
  CONTRACT.md §12.2 Java names, on the same `AxiamClient` as the nine §12
  operations that came before, and added to the `OidcOperations` interface that
  names the vocabulary. Each has a `*Async` `CompletableFuture` companion, the
  per-language exception §12.2's Java note allows. New records
  `FederationProvider` and `FederationProviderList`, matching the §12.1 SDK-type
  table, carrying the `PROTOCOL_*` discriminants and the
  ...
  - **An empty provider list is a success** (§12.1 note 9). An unknown
    ...
    ordinary result and never synthesises a not-found: the endpoint is shaped so
    it cannot enumerate organization or tenant slugs, and distinguishing the
    three client-side would rebuild that oracle. It is therefore also the one
    federation operation that does *not* throw client-side when no workspace
    resolves — a client-side refusal would be that same two-valued answer by
    another route.
  - **`protocol` selects the start operation** (§12.1 note 10), never
    ...
    server-side cannot become a deserialization failure for the whole list.
...
- `AxiamClientLoginProvidersTest` — 20 tests. The wire-shape half reads the
  ...
  empty-list cases, plus that a workspace-less request is still *sent* while
  `ssoStart` refuses the same client), note 10 (all three dispatch branches, with
  a `Saml` fixture whose `provider_kind` is `google` so a kind-based dispatch
  fails), note 12 (terminal `401`, exactly one request) and rule 12a (a `400`
  from either start operation is `NetworkError` and unretried; a `401` from the
  same endpoint stays `AuthError`). The four `*Async` companions are asserted to
  exist and delegate.
...
- Bump com.rabbitmq:amqp-client in the minor-patch group
...
- **§5.2.2 rule 4 is new, and is an errata rather than a wire change.** The
  ...
  eligibility test is host-based; no endpoint is special-cased.
